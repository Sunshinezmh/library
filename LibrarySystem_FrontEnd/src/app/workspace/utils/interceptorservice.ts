...
/*
http://192.168.22.201:8084/authorityManagement-web :权限
http://192.168.22.202:8080/basicInfo-web：基础
默认为 图书馆 http://192.168.22.250:8080/librarysystem
*/
if(typeof url == "string"){
  let str = url.substring(0,url.indexOf("/"));
    switch (str){
       case "authorityManagement-web":
        url = url.replace("authorityManagement-web","http://192.168.22.201:8084/authorityManagement-web");
        break;
      case "basicInfo-web":
        url = url.replace("basicInfo-web","http://192.168.22.202:8080/basicInfo-web");
        break;
      default:
        url = "https://virtserver.swaggerhub.com/Library/Andior/1.0.0/" + url;
        break;
    }
}else{
  let str = url.url.substring(0,url.url.indexOf("/"));
   switch (str){
       case "authorityManagement-web":
        url.url = url.url.replace("authorityManagement-web","http://192.168.22.201:8084/authorityManagement-web");
        break;
      case "basicInfo-web":
        url.url = url.url.replace("basicInfo-web","http://192.168.22.202:8080/basicInfo-web");
        break;
      default:
        url.url = "https://virtserver.swaggerhub.com/Library/Andior/1.0.0/" + url.url;
        break;
    }
}
